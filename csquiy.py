...
class Block(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y):
        ...
        self.rect = self.image.get_rect(center=(pos_x, pos_y))
        self.text = f"{random.choice(helyes if random.random() < 0.5 else hamis)}"

    def update(self):
        self.rect.y += 2
        text(self.text,text_font,(250,250,250),self.rect.x,self.rect.y)
...
def text(text, font, text_col, x, y):
    ...
    for i, line in enumerate(lines):
        ...
        win.blit(img, (x, y + i * line_height))
...
while run:
    ...
    while game:
        ...
        for bullet, blocks in collisions.items():
            for block in blocks:
                if block.text in hamis:
                    score += 1  # Reward a point for shooting a block with a text from the hamis list
                elif block.text in helyes:
                    game = False
